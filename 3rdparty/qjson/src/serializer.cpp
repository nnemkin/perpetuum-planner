...
{
	return serialize( v, QByteArray("\n\t") );
}

QByteArray Serializer::serialize( const QVariant &v, const QByteArray &indent )
{
  QByteArray str;
  bool error = false;
  ...
  if ( ! v.isValid() ) { // invalid or null?
	...
	{
	  QByteArray serializedValue = serialize( v, indent + '\t' );
	...
	str = "[" + indent + join( values, "," + indent ) + indent.left(indent.length() - 1) + "]";
	...
	while ( it.hasNext() ) {
	  it.next();
	  QByteArray serializedValue = serialize( it.value(), indent + '\t' );
	  if ( serializedValue.isNull() ) {
		error = true;
		break;
	  }
	...
	str += join( pairs, "," + indent );
	str += indent.left(indent.length() - 1) + "}";
